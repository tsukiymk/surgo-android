...
inline val MediaMetadataCompat.artUri: Uri
    get() = this.getString(MediaMetadataCompat.METADATA_KEY_ART_URI)?.let { Uri.parse(it) } ?: Uri.EMPTY

inline val MediaMetadataCompat.albumArt: Bitmap?
    get() = getBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART)

inline val MediaMetadataCompat.albumArtUri: Uri
    get() = this.getString(MediaMetadataCompat.METADATA_KEY_ALBUM_ART_URI)?.let { Uri.parse(it) } ?: Uri.EMPTY
...
inline val MediaMetadataCompat.displayIconUri: Uri
    get() = this.getString(MediaMetadataCompat.METADATA_KEY_DISPLAY_ICON_URI)?.let { Uri.parse(it) } ?: Uri.EMPTY

inline val MediaMetadataCompat.mediaUri: Uri
    get() = this.getString(MediaMetadataCompat.METADATA_KEY_MEDIA_URI)?.let { Uri.parse(it) } ?: Uri.EMPTY
...
fun MediaMetadataCompat.toMediaSource(dataSourceFactory: DataSource.Factory) = ProgressiveMediaSource.Factory(dataSourceFactory)
    .createMediaSource(MediaItem.Builder().setUri(mediaUri).build())

fun MediaMetadataCompat.toMediaItem(): MediaItem {
    val builder = MediaItem.Builder()
    builder.setUri(this.mediaUri)
    builder.setMediaMetadata(this.mediaMetadata as MediaMetadata)
